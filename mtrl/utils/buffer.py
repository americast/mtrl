...
class Buffer(SerializedBuffer):
    ...
    def __init__(self, buffer_size, state_shape, action_shape, device):
        ...
        self.states = torch.empty(
            (buffer_size, *(state_shape.shape)), dtype=torch.float, device=device)
        try:
            self.actions = torch.empty(
                (buffer_size, state_shape.shape[0], *(action_shape.shape)), dtype=torch.float, device=device)
        except: pu.db
        self.rewards = torch.empty(
            (buffer_size, state_shape.shape[0]), dtype=torch.float, device=device)
